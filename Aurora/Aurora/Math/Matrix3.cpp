...
namespace Aurora
{
	namespace Math
	{
		...
		void Matrix3::fromHeadPitchRoll(float headDegrees, float pitchDegrees, float rollDegrees)
		{
			...
			float cosH = ACosf(headDegrees);
			float cosP = ACosf(pitchDegrees);
			float cosR = ACosf(rollDegrees);
			float sinH = ASinf(headDegrees);
			float sinP = ASinf(pitchDegrees);
			float sinR = ASinf(rollDegrees);
		...
		void Matrix3::rotate(const Vector3 &axis, float degrees)
		{
			...
			float y = axis.y;
			float z = axis.z;
			float c = ACosf(degrees);
			float s = ASinf(degrees);
